...
def fit_data(filename, temperature, absorbance):
    ...
    if mode == 'hairpin': 
        ...
        #def uv_hairpin_es(t, mss, bss, mds, bds, mds2, bds2, delHconf, delHm, delSm):
        if "1" in filename:   
            p0 = np.array([0.000594, 0.943, 0.000328, 0.8459, 0.000328, 0.8459, 3.13, 62.77, 0.1854])
        else:
            p0 = np.array([0.000594, 0.943, 0.000328, 0.8459, 0.000328, 0.8459, -3.554, 62.77, 0.1854])
        #p0 = np.array([0.000594, 0.943, 0.000328, 0.8459, 0.000328, 0.8459, 3.0, 62.77, 0.1854])
