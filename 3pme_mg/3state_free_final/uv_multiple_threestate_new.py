...
def uv_duplex_es(t, mss, bss, mds, bds, mds2, bds2, delHconf, delSconf, delHm, delSm):
    global R
    print R, Ct
...
def fit_data(filename, temperature, absorbance):
    ...
    if mode == 'hairpin': 
        print "Hairpin"
        #def uv_hairpin_es(t, mss, bss, mds, bds, mds2, bds2, delHconf, delSconf, delHm, delSm):
        p0 = np.array([0.000123, 0.769, -0.0000144, 0.673, -0.0000144, 0.673, -3.554, -0.014446, 69.95, 0.1995])
        #p0 = np.array([0.000123, 0.769, -0.0000144, 0.673, -0.0000144, 0.673, -27.13, -0.092652, 69.95, 0.1995])
        #p0 = np.array([0.000123, 0.769, -0.0000144, 0.673, -0.0000144, 0.673, -2.479, -0.010384, 69.95, 0.1995])
        #p0 = np.array([0.000123, 0.769, -0.0000144, 0.673, -0.0000144, 0.673, 0.0, -0.003777, 69.95, 0.1995])
        #p0 = np.array([0.000123, 0.769, -0.0000144, 0.673, -0.0000144, 0.673, 27.544, 0.014446, 69.95, 0.1995])
        [popt, pcov] = curve_fit(uv_hairpin_es, temperature, absorbance, p0, maxfev=15000)
        ...
        # Unpack popt array
        [mss, bss, mds, bds, mds2, bds2, delHconf, delSconf, delHm, delSm] = popt
        ...
        if delGconf_25 < 0.0:
            ...
            popt = [mss, bss, mds, bds, mds2, bds2, delHconf, delSconf, delHm, delSm]
        ...
        print_result(popt, pcov, ['mss', 'bss', 'mds', 'bds', 'mds2', 'bds2', 'delHconf', 'delSconf', 'delHm', 'delSm'])
        labels = ['mss', 'bss', 'mds', 'bds', 'mds2', 'bds2', 'delHconf', 'delSconf','delHm', 'delSm', 'delGconf_25C', 'delGconf_37C', 'delGm_25C', 'delGm_37C', 'Ct', 'redchi2', 'aic', 'bic']
        result = np.array(list(popt) + [delGconf_25, delGconf_37, delGm_25, delGm_37, Ct, redchi2, aic, bic])

    elif mode == 'duplex':
        print "Duplex"
        #def uv_duplex_es(t, mss, bss, mds, bds, mds2, bds2, delHconf, delSconf, delHm, delSm):
        p0 = np.array([0.0004288, 0.74594, 0.000548, 0.6555, 0.000348, 0.677, -27.1, -0.09265, 83.1, 0.243])
        ...
        print_result(popt, pcov, ['mss', 'bss', 'mds', 'bds', 'mds2', 'bds2', 'delHconf', 'delSconf', 'delHm', 'delSm'])
        labels = ['mss', 'bss', 'mds', 'bds', 'mds2', 'bds2', 'delHconf', 'delSconf', 'delHm', 'delSm', 'delGconf_25', 'delGconf_37', 'delGm_25C', 'delGm_37C', 'Ct', 'redchi2', 'aic', 'bic']
        result = np.array(list(popt) + [delGconf_25, delGconf_37, delGm_25, delGm_37, Ct, redchi2, aic, bic])
...
#mds = 0.000548
#bds = 0.6555
#[t_base, abs_base] = read_data('melt_test2.txt')
#t = np.linspace(5.0, 75.0, 1000) + 273.16
#abs_new = uv_duplex_es(t, mss, bss, mds, bds, mds2, bds2, delHconf, delSconf, delHm, delSm)
#plt.figure(1)
#plt.plot(t-273.16, abs_new)
#plt.plot(t_base-273.16, abs_base, marker='o', color='k', markersize=1, linewidth=0)
#plt.show()
